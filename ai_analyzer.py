...
9. reason 작성 시 가능하면 기술적 시그널(거래량 폭증/돌파/정배열 등)과
   뉴스/펀더멘털 근거를 함께 언급.
10. **sector는 반드시 다음 12개 중 정확히 하나만 사용** (다른 표기 금지):
    - "반도체"          (메모리/비메모리/팹리스 포함)
    - "IT/소프트웨어"   (인터넷/게임/통신/SaaS 포함)
    - "전자/디스플레이" (가전, 전자부품, 디스플레이 패널)
    - "AI/플랫폼"       (AI 솔루션, 데이터 플랫폼)
    - "제약/바이오"     (제약, 바이오, 헬스케어, 의료기기)
    - "자동차"          (완성차, 자동차부품, 전기차, 모빌리티)
    - "화학/소재"       (석유화학, 2차전지 소재, 정유)
    - "에너지"          (전력, 가스, 신재생, 원자력)
    - "금융"            (은행, 증권, 보험, 지주사)
    - "유통/소비재"     (식음료, 화장품, 패션, 미디어/엔터)
    - "건설/조선/기계"  (건설, 조선, 기계, 철강, 방산, 운송)
    - "기타"            (위 어디에도 정확히 들지 않을 때만)
11. **market은 "KOSPI" 또는 "KOSDAQ" 중 하나** (확실치 않으면 종목코드로 판단,
    그래도 모르면 "KOSPI"로). 후처리에서 KIS 응답으로 자동 보정됨.
"""
